...
void setup_tcp_communication() {
  ...
  /*Client wants to send data to server process which is running on server
   * machine, and listening on port on DEST_PORT, server IP address
   * SERVER_IP_ADDRESS. Inform client about which server to send data to : All
   * we need is port number, and server ip address. Pls note that there can be
   * many processes running on the server listening on different no of ports,
   * our client is interested in sending data to server process which is
   * lisetning on PORT = DEST_PORT*/
  dest.sin_port = htons(DEST_PORT);
  struct hostent *host = (struct hostent *)gethostbyname(SERVER_IP_ADDRESS);
  dest.sin_addr = *((struct in_addr *)host->h_addr);

  /*Step 3 : Create a TCP socket*/
  /*Create a socket finally. socket() is a system call, which asks for three
   * paramemeters*/
  ...
  /*Our client is now ready to send data to server. sendto() sends data to
   * Server*/
...
PROMPT_USER:
  ...
  sent_recv_bytes = sendto(sockfd, client_data, sizeof(client_data), 0,
...
#if 0
  /*recvfrom is a blocking system call, meaning the client program will not run
   * past this point untill the data arrives on the socket from server*/
  printf("Waiting for response:\n");
  sent_recv_bytes = recvfrom(sockfd, (char *)&result, sizeof(result_struct_t),
...
#endif
  /*Step 7: Client would want to send the data again to the server, go into
   * infinite loop*/
  goto PROMPT_USER;
}
