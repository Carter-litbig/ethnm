...
void ConnectionManager::SetUdpSocket()
{
  ...
  mreq.imr_multiaddr.s_adr = inet_addr(MULTICAST_ADDR);
  mreq.imr_interface.s_adr = htonl(INADDR_ANY);
...
void ConnectionManager::RecieveMulticast()
{
  ...
  while (1)
  {
    ...
    str_len = recvfrom(master_sock_udp, buf, BUFSIZE - 1, 0, NULL,
                       0); // 브로드캐스트 된 데이터를 수신
    if (str_len < 0u) {
      break;
    }
...
void ConnectionManager::SendMulticast()
{
  ...
  while (true)
  {
    if (sendto(master_sock_udp, msg.c_str(), msg.length(), 0, (sockaddr *)&send_addr,
               sizeof(send_addr)) < 0)
    {
